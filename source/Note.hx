...
class Note extends FlxSprite
{
	...
	public static var scales:Array<Float> = [0.7, 0.65, 0.6, 0.55, 0.5, 0.46];
	public static var swidths:Array<Float> = [160, 140, 120, 110, 100, 90];
	public static var posRest:Array<Int> = [0, 15, 35, 50, 60, 70];
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inEditor:Bool = false)
	{
		...
		if(noteData > -1) {
			...
			*/
			...
			if(!isSustainNote) { //Doing this 'if' check to fix the warnings on Senpai songs
				...
				if (ClientPrefs.barNotes)
					animation.play(Main.gfxLetter[Main.gfxAlterInd[mania][noteData]]);
				else
		...
		if (isSustainNote && prevNote != null)
		{
			...
			if (ClientPrefs.barNotes)
				animation.play(Main.gfxLetter[Main.gfxAlterInd[mania][noteData]] + ' tail');
			else
			...
			{
				if (ClientPrefs.barNotes)
					prevNote.animation.play(Main.gfxLetter[Main.gfxAlterInd[mania][noteData]] + ' hold');
				else
	...
	public function reloadNote(?prefix:String = '', ?suffix:String = '') {
